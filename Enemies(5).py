...
while run:
    ...
    if keys[pygame.K_SPACE] and shootLoop==0:  # facing is changed to change direction of vel. based on which side man is facing
        if man.left:
            facing = -1
        else:
            facing = 1
        ...
        if len(bullets) < 5:  # max 5 bullets at once
            bullets.append(projectile(round(man.x + man.width // 2), round(man.y + man.height // 2), 6, (0, 0, 0),
                                      facing))  # so that it seems that bullet is coming from man,rounded for int value
